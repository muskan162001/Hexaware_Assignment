...
DELETE customer.*, Lease.*, Payment.*
from Customer join lease
on lease.customerId = Customer.customerID

-- 3 Rename the "paymentDate" column in the Payment table to "transactionDate"
ALTER TABLE Payment RENAME paymentDate TO transactionDate;
...
SELECT * FROM Lease WHERE customerID = 2 AND endDate >= CURDATE();

-- 6 Find all payments made by a customer with a specific phone number
SELECT * FROM Payment WHERE leaseID IN (SELECT leaseID FROM Lease WHERE customerID = 555-123-4567);


-- 7 Calculate the average daily rate of all available cars
SELECT AVG(dailyRate) FROM Vehicle WHERE status = 'available';

-- 8 Find the car with the highest daily rate
SELECT * FROM Vehicle WHERE dailyRate = (SELECT MAX(dailyRate) FROM Vehicle);

-- 9 Retrieve all cars leased by a specific customer
SELECT * FROM Vehicle WHERE carID IN (SELECT carID FROM Lease WHERE customerID = 3);
...
SELECT Customer.customerID, Customer.firstName, Customer.lastName, Customer.email, Customer.phoneNumber, ISNULL(SUM(Payment.amount), 0) AS TotalPayments
FROM Customer
...
SELECT Lease.*, Customer.customerID, Customer.firstName, Customer.lastName, Customer.email, Customer.phoneNumber, Vehicle.*
FROM Lease
...
SELECT TOP 1 Customer.customerID, Customer.firstName, Customer.lastName, Customer.email, Customer.phoneNumber, ISNULL(SUM(Payment.amount), 0) AS TotalPayments
FROM Customer
LEFT JOIN Lease ON Customer.customerID = Lease.customerID
LEFT JOIN Payment ON Lease.leaseID = Payment.leaseID
GROUP BY Customer.customerID
...
SELECT Vehicle.*, Lease.*, Customer.customerID, Customer.firstName, Customer.lastName, Customer.email, Customer.phoneNumber
FROM Vehicle
LEFT JOIN Lease ON Vehicle.vehicleID = Lease.vehicleID
